...
namespace diskann {
  ...
  size_t PQFlashIndex<T, TagT>::pipe_search(const T *query1, const _u64 k_search, const _u32 mem_L, const _u64 l_search,
                                            TagT *res_tags, float *distances, const _u64 beam_width,
                                            QueryStats *stats) {
    ThreadData<T> data = this->thread_data_backing_buf[omp_get_thread_num()];
    ...
    // Neighbor nbr_scratch[4096], merged_nbr[4096];
...
// c: search in in-memory index.
    ...
    if (mem_L) {
      ...
      LOG(ERROR) << "mem_L = 0, please use larger value.";
      exit(-1);
    }
#else
    if (mem_L) {
    ...
    auto send_read_req = [&](Neighbor &item) -> bool {
      ...
      const unsigned loc = no_mapping ? item.id : id2loc(item.id);
      const unsigned pid = loc_sector_no(loc);

      uint64_t &cur_buf_idx = query_scratch->sector_idx;
      auto buf = sector_scratch + cur_buf_idx * size_per_io;
      auto &req = query_scratch->reqs[cur_buf_idx];
    ...
    for (_u64 i = 0; i < full_retset.size() && t < k_search; i++) {
      ...
      if (likely(no_mapping)) {
        res_tags[t] = full_retset[i].id;  // use ID to replace tags
      } else {
        tags.find(full_retset[i].id, res_tags[t]);
      }
